...
def dashboard():
    ...
    recent_results = QuizResult.query.filter_by(user_id=current_user.id).order_by(QuizResult.taken_at.desc()).limit(5).all()
...
def view_topic(topic_id):
    ...
    if not resource_data:
        flash('This resource module is being updated with the latest data.', 'info')
